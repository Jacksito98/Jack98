...
Hola Bienvenid@ a 
*${mdata.subject}*
Espero y te sientas agusto en el grupo :D pasala bien y recuerda 😉 Respeta las reglas 🙇🏽‍♂️

Usa ${prefix}verify para verificarte y poder usar el bot no abuses de el.`
  ...
  error: {
  stick: 'Intentalo al rato, tengo problemas tecnicos :(',
  Iv: 'Error en el link'
  },
  only: {
				  ...
				  case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Sigueme en ig', '@Jack_paymon')} ${ran} -o ${ran}`, async (error) => {
