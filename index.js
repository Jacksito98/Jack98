...
+ 'FN:Jack\n' // Nama
+ 'ORG:Jacksito;\n' // Nama bot
+ 'TEL;type=CELL;type=VOICE;waid=18295176783:+1829-517-6783\n' // Nomor bot
+ 'END:VCARD' 
...
Hola Bienvenid@ a 
*${mdata.subject}*
Espero y te sientas agusto en el grupo :D pasala bien y recuerda 😉 Respeta las reglas 🙇🏽‍♂️ o si no te bas a ganar un ban seguro ✅🙂

Usa ${prefix}verify para verificarte y poder usar el bot no abuses de el o te bloqueo 🤝`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Que bueno que te fuiste compay@${num.split('@')[0]} no te queriamos aqui saco eh sal👋`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
  ...
  error: {
  stick: 'Intentalo al rato, tengo problemas tecnicos :( tkm ♥️',
  Iv: 'Error en el link'
  },
  only: {
    ...
    daftarB: `Hola mi loco usa *${prefix}verify* para poder usar el bot`
  }
}
...
☿ sigueme 
_${Ig}_https://www.instagram.com/p/CMeBPOrHB6X/?igshid=ceir0q0gxvjx 
𝐜𝐚𝐫𝐥𝐚 𝐭𝐤𝐦 𝐬𝐨𝐲 𝐭𝐮 𝐬𝐢𝐦𝐩♥️
╔══════════════════♆
║════〘 𝕵𝖆𝖈𝖐𝖘𝖎𝖙𝖔⚔〙═════
║══════════════════♆
╠⛦ *${prefix}Simi (Teks)*
...
║ _Ex : ${prefix}Ephoto 25 Sr.Andy_
╠⛦*${prefix}Textpro list*
║ _Lista de textpro
╠⛦ *${prefix}Textpro (code + Texto)*
║ _Ex : ${prefix}Textpro 55 Sr.Andy _
╚═══════════════════♆
*〘 𝐈𝐌Á𝐆𝐄𝐍𝐄𝐒〙*
╔═══════════════════♆
╠⛦ *${prefix}Cogan*
...
_Gracias pero por ahora no estan activa las donaciones :)_
...
pok = await getBuffer(nimek)
Lxa.sendMessage(from, pok, image, {
  quoted: mek, caption: `Sr.Bot_Jack`
})
				...
				case 'sticker':
				  case 's':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Compay🤦🏽‍♂️ Envía una imagen con un ${prefix}sticker o una etiqueta de imagen que se haya enviado Te va a poner burro? ;-;`)
					}
					break
...
//--Simsimi talk
				case 'simi':
					if (args.length < 1) return reply(`${pushname} Hola que tu quiere compay ;-;`)
					teks = body.slice(6)
					try { 
